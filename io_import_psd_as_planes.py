...
@bpy.app.handlers.persistent#このデコレータはBlenderのハンドラが新しいファイルのロード後も持続する（persistent）ようにします
def register_driver(*args, **kwargs):
    """Register the find_plane_corner function for use with drivers"""#このドライバーは普通にBlenderのきのうのやつ
...
class IMPORT_IMAGE_OT_to_plane(Operator, AddObjectHelper):
    ...
    # -------
    # Draw UI for select dialog
    def draw_import_config(self, context):#contextに表示する場所の情報が含まれている。この関数においては選択ダイアログ内でこの関数が呼び出されるため正しく選択ダイアログ内にこのUIを表示できる
        # --- Import Options --- #
        layout = self.layout
        box = layout.box()
        ...
        box.prop(self, "force_reload")#prop関数トグル式のUIを提供すると同時にself内のそのパラメータを適切に更新する
    ...
    def import_images(self, context):
        ...
        psds = []
        for file in files:
            if file.image.filepath.split('.')[-1] == "psd":
                psd.append(file)
            else:
                print("psd以外は除去されました")#エラーメッセージとして表示

        #psdを処理してカスタムプロパティとして保持するデータとそれに基づいた画像を用意
        psd_infos = []
        for index, psd in enumerate(psds):
            image, psd_info = process_psd.make_psd_data(psd.filepath)
            psds[index].image = bpy.data.images.load(image)
            psd_infos.append(psd_info)
...
# def import_images_button(self, context):#メニューに追加するボタンを作る関数
